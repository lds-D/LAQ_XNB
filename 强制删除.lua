...
endlocal function deleteConstantTerrainByIndex()
...
deleteExhaustRoomChild50()local function deleteVolcanoPitKillParts()
    local map = workspace:FindFirstChild("Map")
    if not map then
        print("Map不存在")
        return false
...
deleteMineshaftIndustryChildren()local function deleteDesolateShelterLava()
    local map = workspace:FindFirstChild("Map")
    if not map then
        print("Map不存在")
        return false
...
deleteLumbermillLava()    local function deleteLumbermillOtherTargets()
        local map = workspace:FindFirstChild("Map")
        if not map then
            print("Map不存在")
            return false
